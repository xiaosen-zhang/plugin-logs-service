...
const columns = [
  ...
  { name: 'dateCreated', defaultFlex: 1,
  minWidth: 200,
  ...
  }, type: 'date', operator: 'before', header: 'Date Created' },
  { name: 'dateUpdated', defaultFlex: 1,
  minWidth: 200,
  filterEditor: DateFilter,filterEditorProps: (props, { index }) => {
    // for range and notinrange operators, the index is 1 for the after field
    return {
  ...
  render: ({ value, cellProps }) => {
    return moment(value).format('MM-DD-YYYY')
  }, type: 'date', operator: 'before', header: 'Date Updated' },
  { name: 'endTime', defaultFlex: 1,
  minWidth: 200,
  filterEditor: DateFilter,filterEditorProps: (props, { index }) => {
    // for range and notinrange operators, the index is 1 for the after field
    return {
  ...
  render: ({ value, cellProps }) => {
    return moment(value).format('MM-DD-YYYY')
  }, type: 'date', operator: 'before', header: 'End Time' },
  { name: 'queueTime', defaultFlex: 1, defaultFlex: 1, header: 'Queue Time' },
  { name: 'startTime',
  minWidth: 200,
  filterEditor: DateFilter,filterEditorProps: (props, { index }) => {
    // for range and notinrange operators, the index is 1 for the after field
  ...
  render: ({ value, cellProps }) => {
    return moment(value).format('MM-DD-YYYY')
  }, type: 'date', operator: 'before', defaultFlex: 1, header: 'Start Time' },
  ...
  render: ({ value, cellProps }) => {
    return moment(value).format('MM-DD-YYYY')
  }, defaultFlex: 1, operator: 'before', type: 'date', header: 'Created At' },
  { name: 'updatedAt',
  minWidth: 200,
  filterEditor: DateFilter,filterEditorProps: (props, { index }) => {
    // for range and notinrange operators, the index is 1 for the after field
  ...
  render: ({ value, cellProps }) => {
    return moment(value).format('MM-DD-YYYY')
  }, defaultFlex: 1, operator: 'before', type: 'date', header: 'Updated At' },
];
...
const loadData = ({ skip, limit, sortInfo, filterValue }) => {
  ...
  const url = 'https://webhook.itsmystaging.com/api/CcvLogs/GetCallLogsList';
